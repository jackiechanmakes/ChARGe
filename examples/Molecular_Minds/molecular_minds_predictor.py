...
class DistanceMessagePassing(MessagePassing):
    ...
    def __init__(self, in_dim: int, out_dim: int):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(in_dim * 2 + 3, out_dim), nn.ReLU(), nn.Linear(out_dim, out_dim)
        )
    ...
    def message(self, x_i, x_j, edge_attr, pos_i, pos_j):
        ...
        distance_features = torch.cat(
            [distance, 1.0 / (1.0 + distance), torch.exp(-distance)], dim=1
        )
...
class LearnedFeatureExtractor(nn.Module):
    ...
    def forward(
        self, atom_embeddings: torch.Tensor, batch_indices: torch.Tensor
    ) -> torch.Tensor:
        ...
        for batch_idx in range(batch_size):
            ...
            if mol_atoms.shape[0] == 0:
                learned_features_list.append(
                    torch.zeros(self.learned_feature_dim, device=atom_embeddings.device)
                )
                continue
            ...
            attention_weights = F.softmax(
                torch.sum(attended_atoms * mol_atoms, dim=1), dim=0
            )
            weighted_mean = torch.sum(
                attended_atoms * attention_weights.unsqueeze(1), dim=0
            )
...
class MultiPropertyMolecularMind(nn.Module):
    ...
    def __init__(
        ...
        self.property_atom_gates = nn.ParameterDict(
            {
                prop: nn.Parameter(torch.ones(atom_feature_dim))
                for prop in property_names
            }
        )
        self.property_molecular_gates = nn.ParameterDict(
            {
                prop: nn.Parameter(torch.ones(molecular_feature_dim))
                for prop in property_names
            }
        )
        self.property_learned_gates = nn.ParameterDict(
            {
                prop: nn.Parameter(torch.ones(learned_feature_dim))
                for prop in property_names
            }
        )
        ...
        self.learned_feature_extractor = LearnedFeatureExtractor(
            atom_embed_dim=hidden_dim, learned_feature_dim=learned_feature_dim
        )
    ...
    def forward(self, batch: Batch) -> Dict[str, torch.Tensor]:
        ...
        explicit_atom_representation = scatter(x, batch.batch, dim=0, reduce="mean")
        ...
        for prop_name in self.property_names:
            ...
            gated_molecular_features = (
                batch.molecular_features * molecular_gates.unsqueeze(0)
            )
            gated_learned_features = learned_features * learned_gates.unsqueeze(0)

            molecular_representation = self.property_molecular_embedding[prop_name](
                gated_molecular_features
            )
            learned_representation = self.property_learned_embedding[prop_name](
                gated_learned_features
            )
            ...
            fused_representation = self.property_fusion[prop_name](
                combined_representation
            )
            predictions[prop_name] = self.property_heads[prop_name](
    ...
    def inverse_transform(
        ...
        for prop_name, pred_tensor in predictions.items():
            if prop_name in self.fitted_properties:
                ...
                denormalized[prop_name] = torch.tensor(
                    denorm_np, device=pred_tensor.device
    ...
    try:
        ...
        for atom in atoms:
            ...
            try:
                ...
                vdw_radii.append(
                    fallback_radii.get(atomic_num, 1.5 + (atomic_num - 1) * 0.05)
                )
        ...
        mol_density = (
            mol_weight / molecular_volume_ml_per_mol
            if molecular_volume_ml_per_mol > 0
            else 1.0
        )
...
def calculate_clean_atom_features(atom, mol) -> List[float]:
    ...
    ring_sizes = [
        len(ring) for ring in mol.GetRingInfo().AtomRings() if atom.GetIdx() in ring
    ]
...
def calculate_clean_molecular_features(mol, coords) -> List[float]:
    ...
    try:
        ...
        if eigenvals[0] > 1e-6:
            ...
            asphericity = (
                eigenvals[0] - 0.5 * (eigenvals[1] + eigenvals[2])
            ) / eigenvals[0]
        else:
            moment_ratio = 0.0
            asphericity = 0.0
        ...
        features.extend(
            [
                ...
                (
                    total_rings / mol_no_hs.GetNumBonds()
                    if mol_no_hs.GetNumBonds() > 0
                    else 0
                ),
            ]
        )
...
def get_clean_bond_features(bond) -> List[float]:
    ...
    features.extend([abs(begin_en - end_en) * 0.1, (begin_en + end_en) * 0.05])

    if bond.IsInRing():
        mol = bond.GetOwningMol()
        ring_info = mol.GetRingInfo()
        bond_rings = [ring for ring in ring_info.BondRings() if bond.GetIdx() in ring]
        if bond_rings:
...
def predict_smiles_batch(
    smiles_list: List[str], predictor: Dict, batch_size: int = 32
) -> List[Dict[str, float]]:
    """
    ...
    for i in range(0, len(smiles_list), batch_size):
        ...
        batch_results = [
            {prop: float("nan") for prop in property_names} for _ in batch_smiles
        ]
